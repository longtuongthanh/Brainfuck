...
RESULT FontShader::Initialize(ID3D11Device* device, HWND hwnd)
{
...
RESULT FontShader::InitShader(ID3D11Device* device,
	HWND hwnd,
	...
	pointDesc[2].SemanticName = "COLOR";
	pointDesc[0].Format = DXGI_FORMAT_R32G32_FLOAT;
	pointDesc[1].Format = DXGI_FORMAT_R32G32_FLOAT;
	pointDesc[2].Format = DXGI_FORMAT_R32G32B32A32_FLOAT;

	pointDesc[0].AlignedByteOffset =
		pointDesc[1].AlignedByteOffset = 
		pointDesc[2].AlignedByteOffset = D3D11_APPEND_ALIGNED_ELEMENT;
	pointDesc[0].InputSlotClass =
		pointDesc[1].InputSlotClass = 
		pointDesc[2].InputSlotClass = D3D11_INPUT_PER_VERTEX_DATA;
	...
	samplerDesc.AddressU = D3D11_TEXTURE_ADDRESS_BORDER;      // D3D11_TEXTURE_ADDRESS_BORDER: outside [0-1] is BorderColor
	samplerDesc.AddressV = D3D11_TEXTURE_ADDRESS_BORDER;      // D3D11_TEXTURE_ADDRESS_CLAMP: outside [0-1] is 0/1
	samplerDesc.AddressW = D3D11_TEXTURE_ADDRESS_BORDER;      // D3D11_TEXTURE_ADDRESS_MIRROR: outside [0-1] is mirrored
...
RESULT FontShader::Release() {
	delete this;
	return 0;
}
RESULT FontShader::Render(ID3D11DeviceContext* deviceContext, int pointCount,
...
void FontShader::OutputShaderErrorMessage(ID3D10Blob* errorMessage,
	HWND hwnd,
...
RESULT FontShader::LoadDrawData(ID3D11DeviceContext* deviceContext,
...
void FontShader::RenderShader(ID3D11DeviceContext* deviceContext, int pointCount)
{
